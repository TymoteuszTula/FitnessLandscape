...
r"""This code contains class for generating and saving random hamiltonians and 
states, and corresponding correlation functions to perform stability analysis 
of theorem 1 and 2. That is done for both finite and zero temperature cases.
"""
...
class StabilityAnalysisSparse:
    r"""Class which provides or saves data for stability analysis of connection
    between correlation functions and Hamiltonians/states. By providing initial
    Hamiltonian, it generates random Hamiltonians or states with, centred 
    around those initial ones, and evaluates correlation functions, metrics and
    ...
    """
    # example of available parameters that can be passed to functions 
    # generate_random_Sij_*
    template_params = {
    ...
    def __init__(self, ham=None, randomizer=None, 
                       corr = ["Sxx", "Sxy", "Sxz", "Syx", "Syy", 
                               "Syz", "Szx", "Szy", "Szz"], 
                       save_rand_ham=False, 
                       temp_mul=None, temp_type="value", no_qpoints=100, 
                       save_Sqs=False, save_Sijs=False, save_rham_params=False,
        ...
        """ constructor, taking the following parameters
			...
			save_rand_ham: bool
			    flag indicating if randomly generated Hamiltonians are saved in 
                output
			temp_mul: float | None
			...
			save_Sqs: bool
			    if True, all structure factors are appended to a list at 
                self.Sqs during run
			save_Sijs: bool
			    if True, all real-space correlators are appended to a list at 
                self.Sijs during run
			save_Sqints: bool
			    if True, all Sq_int (sums of components of structure factor) are
                appended to a list at self.Sqints during run
			save_rham_params: bool
			    if True, sets of parameters for generated random Hamiltonians 
                are appended to a list at self.ham_params during run
			extra_params: dict
    ...
    def run(self, no_of_samples):
        r"""Run algorithm - generate `no_of_samples` times data and append them
        to correct lists.
        ...
        if self.randomizer.rand_ham:
            if self.save_rand_ham:
                data_run = self.generate_random_Sij_sparse_with_hams(
                                no_of_samples)
                self.rhams += data_run["rham"]
                self.NNInfos += data_run["NNInfos"]
            else:
                data_run = self.generate_random_Sij_sparse(no_of_samples)
            self.dist += data_run["dist"]
            self.en += data_run["en"]
            ...
            if self.save_Sijs:
                self.Sijs += data_run["Sijs"]
            if self.save_Sqints:
                self.Sqints += data_run["Sqints"]
            if self.save_rham_params:
                self.ham_params += data_run["ham_params"]  
    ...
    def save_random_samples(self, foldername, filename=None):
        r"""Save lists containing data: dist, diffSij, en, ham_dist to hard 
        drive. Foldername should be of form './.../' and filename should be 
        without /.
        ...
        """
        info = {"L": self.ham.L, "h": self.ham.h, "J": self.ham.J, 
                "delta": self.randomizer.delta, "true_temp": self.ham.temp, 
                "bc": self.ham.bc, "rand_ham": self.randomizer.rand_ham, 
    ...
    def load_random_samples(self, foldername, filename=None):
        r"""Load lists containing data: dist, diffSij, en, ham_dist to hard 
        drive. Foldername should be of form './.../' and filename should be 
        without /.
    ...
    def generate_random_Sij_sparse(self, no_of_samples, params={}):
        r"""Generate data for a given no of samples:
			This routine provides an algorithm for randomly sampling the 
            landscape of the fitness function (deviation from target structure 
            factor)
        Parameters
        ----------
			no_of_samples: int
        ...
        #H_in = create_hamiltonian_sparse(L, params_input={"L": self.L, "J": 
        # self.J, "h": self.h})
        ...
        exp_fac = CorrelationCalculator.calculate_exp_fac(self.ham.L, 
                                                          self.no_qpoints)
        Lambdas = CorrelationCalculator.calculate_Lambdas(self.ham.L)
        if self.ham.temp == 0:
            en_in, gs = SijCalculator.find_gs_sparse(H_in)
            state_in = gs[:,0]
            #Sij_in = SijCalculator.return_Sij(self.ham.L, state_in, SX, SY, 
            #               SZ, self.ham.temp)
            Sij_in, Sq_in, Sq_int_in = SijCalculator.return_Sq2(self.ham.L, 
                    state_in, SX, SY, SZ, self.ham.temp, 
            ...
            #Sij_in = SijCalculator.return_Sij(self.ham.L, state_in, SX, SY, 
            # SZ, self.ham.temp)
            Sij_in, Sq_in, Sq_int_in = SijCalculator.returnSq2_dm_not_sparse(
                self.ham.L, state_in, SX, SY, SZ, no_ofqpoints=self.no_qpoints,
        ...
        # seeds = [[np.random.randint(1000)] for i in 
        # range(self.randomizer.no_of_processes)]
        with Pool(processes=self.randomizer.no_of_processes) as pool:
            ...
            data = list(tqdm.tqdm(pool.imap_unordered(
                self.randomizer.return_random_state, iter), total=len(iter)))
        stop_time = time.time()
        print ("Completed sampling random instances in time ", 
               stop_time-start_time, flush=True)
    ...
    def generate_random_Sij_sparse_with_hams(self, no_of_samples):
        ...
        """  
        #H_in = calculate_ham_sparse(L, states, h, J_onsite, J_nn, J_nnn, bc)
        #H_in = create_hamiltonian_sparse(L, params_input={"L": self.L, 
        # "J": self.J, "h": self.h})
        H_in = self.ham.get_init_ham()
        SX = create_sx_sparse(self.ham.L)
        SY = create_sy_sparse(self.ham.L)
        ...
        if self.ham.temp == 0:
            ...
            Sij_in, Sq_in, Sq_int_in = SijCalculator.return_Sq2(self.ham.L, 
                state_in, SX, SY, SZ, self.ham.temp, 
                no_ofqpoints=self.no_qpoints, exp_fac=exp_fac, Lambdas=Lambdas)
        else:
            en_in, _ = SijCalculator.find_gs_sparse(H_in)
            state_in = SijCalculator.return_dm_not_sparse(H_in, 1/self.ham.temp)
            #Sij_in = SijCalculator.return_Sij(self.ham.L, state_in, SX, SY, 
        ...
        start_time = time.time()
        # seeds = [[np.random.randint(1000)] for i in range(
        # self.randomizer.no_of_processes)]
        with Pool(processes=self.randomizer.no_of_processes) as pool:
            seeds = np.random.randint(1000, size=self.randomizer.no_of_processes)
            for i in range(self.randomizer.no_of_processes):
                pool.apply_async(self.set_seed, (seeds[i],))
            params = [{"H_in": H_in, "SX": SX, "SY": SY, "SZ": SZ, "en_in": en_in, 
                      ...
                      "Lambdas": Lambdas, "Sq_int_in": Sq_int_in, 
                      "no_qpoints": self.no_qpoints}]
            iter = no_of_samples * params
            # seeds = np.random.randint(1000, size=4)
            # for j in range(self.randomizer.no_of_processes):
            #     iter[j]["seed"] = seeds[j]
