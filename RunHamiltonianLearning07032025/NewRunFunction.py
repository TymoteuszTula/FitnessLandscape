...
def get_filename2(parameters):
	""" Quick method to get only the parameters that we care about (L and temp)"""
	filename = "L_{}_T_{}_RandHamRun07032025".format(parameters['L'], parameters['temp'])
	return filename

def single_run(L, temp, samples, processes):
	...
	no_of_processes= processes # <-- Number of processes to use (parallelization)
    ...
    # Change the random initial Hamiltonian so that it has the same randomisation
	# as done in the randomizer (translational invariance is still preserved)
	J = [[J_max_value*(1.0-1.0*np.random.randn(3, 3)) + 0.0]]
    ...
    # Set temperature as the average value of the randomly set Js
	temp_ratio = temp
	temp = temp * np.average(J)
	...
	# CREATE RANDOMIZER:
	# Write new parameters for randomisation of Ham
	delta_ham = [0.5, 0.5]

	# RANDOMIZER = randomizer.RandomizerStateRandomDelta(
	# 	HAMILTONIAN, delta, no_of_processes, random_distribution_type)
	RANDOMIZER = randomizer.RandomizerHamiltonianRandomRandomDelta(
		HAMILTONIAN, delta_ham, no_of_processes
	)
	...
	dist, en, diffSij, diffSq, dist_ham, Sqs, Sijs, Sqints, Sq_int, ham_params=STABILITY_ANALYSIS.generate_random_Sij_sparse(no_of_samples)
	...
	filename=get_filename2(parameters)

	outpath='./RunHamiltonianLearning07032025/data/'+filename+'.pickle'
